...
from src.PulsePalGUI import MainWindow
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--port', help='the serial port used to communicate with the Pulse Pal. If not provided, '
                                             'then the software will try to find the port automatically, and/or offer '
                                             'a choice of possible ports')
    parser.add_argument('-v', '--verbose', action='count', default=0, help="increase verbosity of output (can be "
                                                                           "repeated to increase verbosity further)")
    args = parser.parse_args()

    level = LOGGING_LEVELS[min(args.verbose, len(LOGGING_LEVELS) - 1)]  # cap to last level index
    ...
    if args.port is None:
        ...
        if len(possible_ports) > 1:
            ...
            item, ok = QInputDialog().getItem(None, "Choose the correct serial port",
                                              "Serial ports:", items, 0, False)
            if ok:
    ...
    if args.port is None:
        raise PulsePalError('Could not find a suitable serial port. Please provide the serial port using the '
                            '--port argument')
